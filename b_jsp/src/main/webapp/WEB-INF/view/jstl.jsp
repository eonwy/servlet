...
<div>jsrl Array : <c:out value="${colors}"/></div>

<h4>3. 조건문</h4>
<h5>if</h5>
<c:if test="${kor < math}">
    <div>수학을 더 잘하시는 군요!</div>
</c:if>
<c:if test="${kor > math}">
    <div>국어를 더 잘하시는 군요!</div>
</c:if>
...
<ul>
    <c:forTokens var="token" items="java html css js mysql servlet" delims=" ">
        <li>
            <c:out value="${token}" />
        </li>
    </c:forTokens>
